...
class ChatGPTTelegramBot:
    ...
    def __init__(
        ...
        if self.config.get('enable_transcription', True):
            self.commands.append(BotCommand(command='transcribe', description=localized_text('transcribe_description', bot_language)))
    ...
    async def transcribe_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Prompts the user to send a voice / audio / video file for transcription.
        The actual transcription is performed by the existing media handler.
        """
        ...
        await update.effective_message.reply_text(
            ...
            text=localized_text('transcribe_prompt', bot_language)
        )
        ...
        async def _execute():
            ...
            try:
                ...
                if self.config['voice_reply_transcript'] and not response_to_transcription:
                    ...
                    response, total_tokens = await self.openai.get_chat_response(chat_id=chat_id, query=transcript, user_id=user_id)
    ...
    async def vision(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async def _execute():
            ...
            if self.config['stream']:
                ...
                stream_response = self.openai.interpret_image_stream(chat_id=chat_id, fileobj=temp_file_png, prompt=prompt, user_id=user_id)
                i = 0
                prev = ''
                sent_message = None
                backoff = 0
                ...
                try:
                    interpretation, total_tokens = await self.openai.interpret_image(chat_id, temp_file_png, prompt=prompt, user_id=user_id)
