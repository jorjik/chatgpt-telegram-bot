...
2. Send the media; the bot extracts audio via `pydub`, sends it to Whisper, and replies with the transcript.
...
5. The bot extracts audio, transcribes with Whisper (`verbose_json` with timestamps), asks the LLM (OpenAI or Anthropic, same `LLM_PROVIDER` env as `/brief`) to pick the best self-contained highlights, then for each highlight runs a single `ffmpeg` pass that cuts the range, center-crops to 9:16 @ 1080×1920, and (optionally) burns SRT subtitles with a classic white + black-outline style.
